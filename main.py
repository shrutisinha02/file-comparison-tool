...
def send_email_report():
    try:
        ...
        if current_view == 'changes' and last_change_report is not None:
            filename = "Comparison_Report.xlsx"
            data_to_send = last_change_report
            msg['Subject'] = 'Comparison Report'
            msg.set_content("Please find the attached comparison report.")
        elif current_view == 'new_joinees' and new_joinees_report is not None:
            filename = "New_Joinees_Report.xlsx"
            data_to_send = new_joinees_report
            msg['Subject'] = 'New Joinees Report'
            msg.set_content("Please find the attached list of new joinees.")
        else:
            messagebox.showinfo("No Data", "No valid report to email. Please run comparison or show new joinees.")
            return

        msg['From'] = sender_email
        msg['To'] = receiver_email

        temp_path = f"temp_report_{filename}"
        data_to_send.to_excel(temp_path, index=False)
        ...
        messagebox.showinfo("Email Sent", f"{filename} sent to {receiver_email} successfully!")
